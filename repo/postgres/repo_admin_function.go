...
func (s adminFunctionRepository) GetOrderAll(ctx context.Context) ([]admin_model.ResOrderHistory, error) {
	var resOrderHistory []admin_model.ResOrderHistory
	type Result struct {
	...
	resOrderHistoryMap := make(map[int64]*admin_model.ResOrderHistory)
	...
	for _, result := range results {
		if item, exists := resOrderHistoryMap[result.OrderID]; exists {
			...
			resOrderHistoryMap[result.OrderID] = &admin_model.ResOrderHistory{
